...
func (srv *Server) Serve() (err error) {
	...
	if err = srv.Server.Serve(srv.ln); err != nil && err != http.ErrServerClosed {
		logs.Emergency(syscall.Getpid(), "Server.Serve() error:", err)
		return err
	}
...
func (srv *Server) ListenAndServe() (err error) {
	...
	logs.Info("Gon Web App starting\nGon " + srv.ServerName + "\nWeb App Name: "+srv.AppName +"\nGon Server starting in: \n * Environment: %s\n * Listening:   http:://%s\n * Datetime:    %s", srv.EnvMode, addr, time.Now().Format("2006-02-01 15:04:05.123"))
	return srv.Serve()
}
...
func (srv *Server) getListener(laddr string) (l net.Listener, err error) {
	if srv.isChild {
		...
		if len(socketPtrOffsetMap) > 0 {
			...
			logs.Info("laddr", laddr, "ptr offset", socketPtrOffsetMap[laddr])
		}
...
func (srv *Server) handleSignals() {
	...
	for {
		...
		switch sig {
		case syscall.SIGHUP:
			logs.Info(pid, "Received SIGHUP. forking.")
			err := srv.fork()
			if err != nil {
				logs.Error("Fork err:", err)
			}
		case syscall.SIGINT:
			logs.Info("===== ", pid, "Received SIGINT.", " ===== ")
			srv.shutdown()
		case syscall.SIGTERM:
			logs.Info("===== ", pid, "Received SIGTERM.", " ===== ")
			srv.shutdown()
		default:
			logs.Warn("Received %v: unrecognized signal\n", sig)
		}
...
func (srv *Server) shutdown() {
	...
	logs.Info("- Gracefully stopping, waiting for requests to finish")
	logs.Info("- Osu!")
...
func (srv *Server) fork() (err error) {
	...
	err = cmd.Start()
	if err != nil {
		logs.Emergency("Restart: Failed to launch, error: %v", err)
	}
